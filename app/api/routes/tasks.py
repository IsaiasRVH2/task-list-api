...
async def create_task_endpoint(task_create: TaskCreate, session: AsyncSession = Depends(get_session)):
    task = await create_task(task_create, session)
    if not task:
        return {"error": "Task creation failed."}
    return task

@router.get("/tasks/{user_id}", response_model=List[TaskResponse])
async def get_tasks_by_user_endpoint(user_id: int, session: AsyncSession = Depends(get_session)):
...
async def update_task_endpoint(task_id: int, task_update: TaskUpdate, session: AsyncSession = Depends(get_session)):
    task = await update_task(task_id, task_update, session)
    if not task:
